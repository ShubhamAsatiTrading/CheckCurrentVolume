...
def process_single_file(filename, config_values):
    """
    Process a single CSV file - extracted for parallel execution
    
    Args:
        filename (str): Name of the CSV file to process
        config_values (dict): Configuration values from common.txt
        
    Returns:
        tuple: (symbol, result_dict) or (symbol, None) if failed
    """
    try:
        ...
        data_file_path = os.path.join("aggregated_data", filename)
        ...
        if os.path.exists(historical_file_path):
            ...
            historical_df = None
            print(f"⚠️  Warning: Historical data file not found: {historical_file_path}")
        ...
        print(f"✅ Completed: {symbol} ({backtest_results['total_trades']} trades)")
        return symbol, result
        
    except Exception as e:
        print(f"❌ Error processing {filename}: {str(e)}")
        return parse_filename(filename), None


def run_backtest():
    """
    Main function to run backtesting for all files in aggregated_data folder
    NOW WITH MULTITHREADING - Automatically detects and processes all CSV files
    ...
    print(f"🚀 Found {len(csv_files)} CSV files - Processing with 3 workers")
    for f in csv_files:
        print(f"  - {f}")
    
    # Step 2: Read configuration values from common.txt
    config_values = read_ohlc_from_common()
    print(f"\n⚙️  Configuration: {config_values}")
    
    all_results = {}
    processed_count = 0
    failed_count = 0
    
    # Step 3: Process files in parallel with ThreadPoolExecutor
    print(f"\n🔥 Starting parallel processing...")
    
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all files for processing
        future_to_filename = {
            executor.submit(process_single_file, filename, config_values): filename 
        ...
        for future in as_completed(future_to_filename):
            ...
            try:
                ...
                print(f"📊 Progress: {total_completed}/{len(csv_files)} completed")
...
if __name__ == "__main__":
    print("=== Testing Backtest Engine with Multithreading ===")
