...
# ================================================================================
# MINIMAL CONCURRENT PROCESSING - ULTRA CONSERVATIVE
# ================================================================================

def process_worker(args):
    """Simple worker for concurrent processing."""
    file_path, interval_minutes, interval_days, threshold_mapping = args
    filename = os.path.basename(file_path)
    symbol = extract_symbol_from_filename(file_path)
...
def process_all_files(interval_minutes: int, 
                     ...
                     enable_concurrent: bool = True) -> List[Dict[str, Any]]:
    """
    ...
    print(f"Aggregating to {interval_minutes}-minute, {interval_days}-day intervals...")
    
    # Conservative worker count: Max 2-4, 50% CPU usage
    max_workers = max(1, min(4, int((os.cpu_count() or 2) * 0.5)))
    
    if enable_concurrent and len(csv_files) > 1:
        print(f"Concurrent processing: {max_workers} workers (conservative)")
    else:
    ...
    worker_args = [(file_path, interval_minutes, interval_days, threshold_mapping) for file_path in csv_files]
                ...
                for i, future in enumerate(as_completed(futures), 1):
                    try:
                        ...
                        if aggregated_data is None:
                            ...
                            if save_files:
                                save_single_aggregated_file(aggregated_data, output_folder)
