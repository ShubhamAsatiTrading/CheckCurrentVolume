...
def consolidate_volume_boost_data(interval_minutes=0,
    ...
    # Process files in parallel - MAIN OPTIMIZATION
    last_rows = []
    lock = threading.Lock()
...
def send_telegram_signal(bot_token, chat_id, message):
    """Send signal to Telegram with retry for rate limits"""
    try:
...
def live_market_scanner(kite_instance, telegram_bot_token, telegram_chat_id, 
    ...
    # Step 5: Compare and generate signals - PARALLEL TELEGRAM SENDING
...
🕒 <b>Current Time:</b> {current_time}
                ...
                signals_to_send.append((symbol, message))
    
    # Send signals in parallel
    signals_sent = 0
    
    def send_signal(signal_data):
        symbol, message = signal_data
    ...
    if signals_to_send:
        with ThreadPoolExecutor(max_workers=25) as executor:
            results = list(executor.map(send_signal, signals_to_send))
            signals_sent = sum(results)
